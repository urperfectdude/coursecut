...
//
// This is the half of plan §6's isolation model that RLS cannot do for
// itself: **a policy has no effect on a superuser, and none on a table's
// owner unless the table is FORCEd.** Enabling RLS while still connecting as
// `postgres` produces a database that looks protected in `\d` and is not. So
// the app gets its own role here, and `0001_rls.sql` FORCEs every policy so
// even an ownership mistake later cannot quietly re-open the door.
//
...
//
//   npm run db:bootstrap        # after `docker compose up`, before db:migrate
...
/**
 * The same grants over `graphile_worker`, once it exists (M5).
 *
 * Both runtime processes need it: `apps/api` calls `add_job` inside a request
 * transaction, and `apps/worker` locks, completes and fails jobs. Neither may
 * be the admin role — a superuser ignores RLS outright, which would quietly
 * undo `0001_rls.sql` for the two processes that touch the most tenant data.
 * So the unprivileged role gets working rights on the queue and still no DDL.
 *
 ...
 *
 * These tables carry no `org_id`. That is not a hole: a queue entry holds a job
 * id and an org id in its payload, and every handler resolves that through
 * `withOrg` before reading a single tenant row. The queue schedules work; it
 * never answers questions about tenant data.
 */
async function grantQueuePrivileges(client: pg.Client, appUser: string, owner: string): Promise<void> {
  ...
  // where `public`'s policies decide what it can actually see. Scoping the
  // queue itself would protect nothing and break `add_job`.
  //
  ...
  await client.query(
    `DO $$
     ...
     BEGIN
       ...
       LOOP
         IF NOT EXISTS (
           SELECT 1 FROM pg_policy WHERE polrelid = target AND polname = 'coursecut_app_all'
         ) THEN
           EXECUTE format(
             'CREATE POLICY coursecut_app_all ON %s FOR ALL TO %I USING (true) WITH CHECK (true)',
...
export async function bootstrap(): Promise<void> {
  ...
  try {
    ...
    // are spelled out because every one of them being false is what makes the
    // policies in `0001_rls.sql` mean anything.
