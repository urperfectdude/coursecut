...
// creating tables and indexes is not something the request-serving role is
// allowed to do (see `src/env.ts`).
...
export default defineConfig({
  ...
  dialect: "postgresql",
  dbCredentials: {
    url: process.env.DATABASE_ADMIN_URL ?? "postgres://postgres:postgres@localhost:5432/stepcut",
  },
  ...
  // would be called. snake_case is the convention inherited from coursecut.
  casing: "snake_case",
  verbose: true,
  strict: true,
});
