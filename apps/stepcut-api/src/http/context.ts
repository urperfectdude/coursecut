...
//
// Copied from apps/api/src/http/context.ts. Every route below this runs
// inside `withOrg()` once a tenant table exists, and `withOrg()` is only as
...
// value the RLS mechanism will key on once it exists, and it is written by a
// code path the client can drive. One membership lookup per request is a
// cheap price for the property that a stale or tampered session id cannot
// select a tenant.
//
// A session with zero memberships (fresh signup, before `CreateOrgScreen`
// runs) hits `adoptDefaultOrg` below, finds nothing, and gets a real 403 —
// not an empty result. `apps/web/src/auth/useOrgs.ts` is the intended reader
// of that 403: it treats it as "no orgs yet" rather than an error, so the
// signed-in-with-no-org state renders `CreateOrgScreen` instead of failing.
// `apps/stepcut`'s `useOrgs.ts` (step 3) copies that same translation.
 ...
 * memberships at all is a real 403 — a future RLS policy would return empty
 * rows instead, which looks identical to having lost everything.
 */
 ...
 * literal in the same expression. It cannot actually be missing — the route
 ...
 * handler will touch a tenant table, once one exists.
 */
