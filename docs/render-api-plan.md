...
| Exact route/request shape for the future click-based add-on (a new endpoint vs. a `mode` field on `POST /v1/walkthroughs`), and whether it shares `walkthrough_jobs`/`walkthrough_job_steps` or gets its own tables | Leaning a separate endpoint over a `mode` field — the two input shapes (narration+video vs. video+click-log) don't share a request body, so branching inside one route would mostly be an if/else splitting two unrelated implementations. Table reuse is a smaller decision, revisit when this is actually scheduled |
| Whether the extension-side work (emitting `t_ms`/`x`/`y`/`selector`/`element_text`/`tag_name`) is scoped anywhere yet | Not yet — this is still a dependency the future add-on would need, unscoped as its own task |
